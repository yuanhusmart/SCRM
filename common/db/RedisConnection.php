...
class RedisConnection extends Component
{
    ...
    const EVENT_BEFORE_REDIS_QUERY = 'before.redis.query';

    const EVENT_AFTER_REDIS_QUERY = 'after.redis.query';
    ...
    public function __call($name, $params)
    {
        ...
        $this->redisCommand = ucfirst(strtolower($redisCommand));
        $this->params       = $params;
        // 开始
        $this->trigger(self::EVENT_BEFORE_REDIS_QUERY);
        if (in_array($redisCommand, $this->redisCommands)) {
            $result = $this->executeCommand($redisCommand, $params);
            // 结束
            $this->trigger(self::EVENT_AFTER_REDIS_QUERY);
            return $result;
        }
