...
use common\models\SuiteCorpProgramNextCursor;

class SuiteCorpProgramNextCursorService extends Service
{
    ...
    {
        $data = SuiteCorpProgramNextCursor::findOne(['suite_id' => $suiteId, 'corp_id' => $corpId]);
        if (!$data) {
            $data = new SuiteCorpProgramNextCursor();
        }
    ...
    {
        $redisKey   = \Yii::$app->params["redisPrefix"] . 'program.next.cursor.' . $suiteId . '.' . $corpId;
        $nextCursor = \Yii::$app->redis->get($redisKey);
        if (empty($nextCursor)) {
            $nextCursor = SuiteCorpProgramNextCursor::find()->where(['suite_id' => $suiteId, 'corp_id' => $corpId])->select('next_cursor')->scalar();
        }
        return $nextCursor;
    }
